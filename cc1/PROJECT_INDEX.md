...
- `backend/thema_ads_service.py` - Business logic and job processing
  ...
  - `activate_ads_per_plan_v2()` - V2 AD-FIRST: 10-100x faster activation (lines 2266-2512); revolutionary approach queries FROM ad_group_ad_label instead of ad groups; two-step label resolution gets label.id by name then queries ads directly by label.id; only 4 queries per customer (get theme label ID, query theme ads, get original label ID, query original ads); THEMA_ORIGINAL ads query batched (1000 ad groups per batch) to avoid FILTER_HAS_TOO_MANY_VALUES on large customers; parallel customer processing with configurable workers; batch mutations (5000 ads per chunk); **THREE CRITICAL FIXES (2025-11-24)**: (1) Only enables ONE theme ad per ad group (break statement line 2690) to avoid RSA 3-ad limit violations; (2) Queries FROM ad_group_ad_label with direct label filtering (lines 2605-2617) - no subqueries due to GAQL limitations; (3) Uses dict storage for deduplication (lines 2594-2642) to prevent ID_EXISTS_IN_MULTIPLE_MUTATES errors when ads have multiple labels
...
├── run_activation.py                # Utility: Direct activation runner bypassing HTTP endpoint; uses activate_ads_per_plan_v2() with all optimizations; useful for testing and debugging activation without frontend; standalone Python script with async support
├── activate_ads_v2.py               # Utility: Standalone V2 activation module with clean implementation reference; ad-first query approach; includes deduplication, batch operations, and all critical fixes; use as reference implementation for activation logic
